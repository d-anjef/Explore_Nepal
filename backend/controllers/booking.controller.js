...
export const bookPackage = async (req, res) => {
  try {
    ...
    // Prevent admin from booking packages
    ...
    return res.status(500).send({
      success: false,
      message: "Error booking package: " + error.message,
    });
  }
};
...
export const getCurrentBookings = async (req, res) => {
  try {
    ...
    // Filter bookings based on tour end date (start date + duration)
    const today = new Date();
    today.setHours(0, 0, 0, 0);
    
    let bookingsFilterd = [];
    bookings.forEach((booking) => {
      if (booking.buyer !== null && booking.packageDetails !== null) {
        const bookingDate = new Date(booking.date);
        bookingDate.setHours(0, 0, 0, 0);
        
        // Calculate tour end date (start date + package days)
        const tourEndDate = new Date(bookingDate);
        const packageDays = booking.packageDetails.packageDays || 1;
        tourEndDate.setDate(tourEndDate.getDate() + packageDays);
        
        // Only include if tour hasn't ended yet
