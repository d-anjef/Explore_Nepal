...
export const deleteUserAccountAdmin = async (req, res, next) => {
  try {
    ...
    // Delete all guide messages sent by this user (as a user requesting guides)
    await GuideMessage.deleteMany({ userEmail: user.email });

    // Delete all guide messages received by this user (if they are a guide)
    await GuideMessage.deleteMany({ guideEmail: user.email });

    // Delete guide application submitted by this user
    await GuideJobApplication.deleteMany({ email: user.email });
    ...
    res.status(200).send({
      success: true,
      message: "User account and all related data have been deleted!",
    });
  } catch (error) {
    console.log(error);
    res.status(500).send({
